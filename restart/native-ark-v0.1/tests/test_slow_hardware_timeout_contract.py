...
class SlowHardwareTimeoutContractTests(unittest.TestCase):
    def test_boot_proof_wrapper_raises_default_http_budget_to_five_minutes(self):
    ...
    def test_runtime_router_allows_five_minute_agent_response(self):
        dropin = (ROOT / "rootfs/etc/systemd/system/arkd.service.d/20-slow-hardware-timeout.conf").read_text()
        self.assertIn("ARK_AGENT_RESPONSE_TIMEOUT_SECONDS=300", dropin)
