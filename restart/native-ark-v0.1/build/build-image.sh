...
if [[ "${ARKLINUX_PRIVATE_BUILD_NAMESPACE:-0}" != "1" ]]; then
  # Give the assembler a catchable shutdown signal so its EXIT teardown can
  # release guest mounts, device-mapper mappings, and the loop device.
  exec unshare --mount --pid --fork --mount-proc --kill-child=SIGTERM \
...
validate_guest_contract no_baked_agent_private_material 'for role in kyle aletheia joey hrm kenny; do private="/ark/agents/$role/private"; ! test -L "$private" || exit 1; if test -e "$private"; then test -d "$private" || exit 1; test -z "$(find "$private" -mindepth 1 -print -quit)" || exit 1; fi; done'
