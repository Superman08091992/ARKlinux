...
request only the explicit operations implemented below.  New privileged UI
features must add a named, validated operation rather than an arbitrary argv,
file-read, or file-write escape hatch.
"""
...
def run(argv: list[str], timeout: int = 30) -> dict[str, Any]:
    ...
    return {
        ...
        "stdout": cp.stdout[-256_000:],
        "stderr": cp.stderr[-64_000:],
    }
...
def dispatch(req: dict[str, Any]) -> dict[str, Any]:
    ...
    if op == "journal":
        ...
        return run([
            "/usr/bin/journalctl", "-u", unit, "-n", str(count),
    ...
    if op == "set_hostname":
        hostname = str(req.get("hostname") or "").strip()
        if not HOSTNAME_RE.fullmatch(hostname) or ".." in hostname:
            raise ValueError("invalid hostname")
        return run(["/usr/bin/hostnamectl", "set-hostname", hostname], 15)
